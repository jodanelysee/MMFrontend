        ...
        .then(async data => {
          ...
          const ordersWithDetails = await Promise.all(
            data.map(async order => {
              const cardResponse = await fetch(`https://mmbackend-zv50.onrender.com/card/${order.card}`);
              const cardData = await cardResponse.json();
              // Fetch user details to get shipping address
              const userResponse = await fetch(`https://mmbackend-zv50.onrender.com/admin/${userID}`);
