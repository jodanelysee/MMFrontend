...
const ManageCards = () => {
  ...
  useEffect(() => {
    ...
    if (!userID) {
      ...
      fetch(`https://mmbackend-zv50.onrender.com/all-cards?userID=${userID}`).then(res => res.json()).then(data => setAllCards(data));
  }, [])

  // delete card
  const handleDelete = (id) => {
    console.log(id)
    fetch(`https://mmbackend-zv50.onrender.com/card/${id}`,{
        method: "DELETE",
